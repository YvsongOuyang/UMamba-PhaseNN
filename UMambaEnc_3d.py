...
from utils import PhiLayer, ObjLayer, SupportLayer, MaskedObjLayer, FarfieldDiffLayer
...
class UMambaEnc(nn.Module):
    ...
    def forward(self, x):
        ...
        amp = torch.sigmoid(self.decoder1(skips))
        ph_raw = torch.tanh(self.decoder2(skips))
    ...
    def compute_conv_feature_map_size(self, input_size):
        ...
        return (
            ...
            + self.decoder1.compute_conv_feature_map_size(input_size)
            + self.decoder2.compute_conv_feature_map_size(input_size)
        )

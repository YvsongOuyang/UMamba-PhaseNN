...
def get_cropped_module_phase_high_strain(obj,
                                        threshold_module=None,
                                        crop=False,
                                        apply_fftshift=False,
                                        unwrap=True) :
    '''
    High strain version of get_cropped_module_phase. Not sure that it wrks well for all particles.
    '''
    # compute phase gradient
    module, phase = get_cropped_module_phase(obj, threshold_module=0., 
                                             crop=crop, apply_fftshift=apply_fftshift, unwrap=unwrap)
    phase_grad = np.array(np.gradient(phase))
    phase_grad = np.sqrt(np.sum(phase_grad**2., axis=0))
    
    # Create a renormalized modulus
    factor = np.copy(phase_grad) 
    factor[factor > 2.] = 2. # hard coded threshold in phase gradient
    module_custom = module*(1.+factor*2.)
    
    # Use a support from module_custom
...
def get_complex_conjugate(obj):
    obj_conj = ifftshift(ifftn(np.conj(fftn(fftshift(obj)))))
    return obj_conj
...
def automatic_object_roi_on_support(obj,
                                    threshold_module=.3,
                                    plot=False):
    '''
    Useful for low photon BCDI data (like BM02)
    '''
    support = create_support(obj, threshold_module=threshold_module)
    roi = automatic_object_roi(support, plot=plot)
    return roi
...
def pad_to_equal_shape(obj,
                       target = None,
                       verbose=True):
    '''
    from object of shape (n1,n2,n3), pad to the shape (nmax,nmax,nmax) where nmax=max(n1,n2,n3)
    '''

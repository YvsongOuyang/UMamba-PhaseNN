...
def prepare_author_training_sample(
    ...
    *,
    ...
    if return_refinement_targets:
        if "object" not in stored or "support" not in stored:
            raise ValueError(
                "Real-space refinement requires compact/extras NPZ fields "
                "'object' and 'support'."
            )
        realspace = np.asarray(stored["object"])
        support = np.asarray(stored["support"], dtype=bool)
        if (
            realspace.shape != shape
            or not np.iscomplexobj(realspace)
            or not np.isfinite(realspace).all()
            or support.shape != shape
        ):
            raise ValueError("Invalid real-space object/support refinement target.")
        diffraction_modulus = np.sqrt(intensity, dtype=np.float32)
        diffraction_modulus /= float(diffraction_modulus.max())
        sample.update(
            {
                "diffraction": torch.from_numpy(
                    np.ascontiguousarray(diffraction_modulus[None])
                ),
                "realspace": torch.from_numpy(
                    np.ascontiguousarray(realspace.astype(np.complex64, copy=False))
                ),
                "support": torch.from_numpy(np.ascontiguousarray(support)),
            }
        )
    return sample
...
class AuthorNPZPhaseDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        self.return_refinement_targets = bool(return_refinement_targets)
        if min_oversampling is not None and (
            not np.isfinite(min_oversampling) or min_oversampling <= 0
        ):
        ...
        if manifest.get("split_unit") != "particle" or split not in manifest.get(
            "splits", {}
        ):
            raise ValueError(
                "Generate particle-disjoint splits with --split-counts TRAIN VAL TEST."
            )
        ...
        for record in records:
            ...
            if previous != record_split:
                raise ValueError(
                    f"Particle leakage between {previous} and {record_split}."
                )
            counts[record_split] = counts.get(record_split, 0) + 1
            eligible = True
            if min_oversampling is not None:
                measured = np.asarray(
                    metadata.get("measured_object_oversampling_xyz"), dtype=np.float64
                )
                if (
                    measured.shape != (3,)
                    or not np.isfinite(measured).all()
                    or np.any(measured <= 0)
                ):
                    raise ValueError(
                        f"Missing or invalid oversampling metadata for {name}; cannot apply filter."
                    )
        ...
        if any(counts.get(key, 0) != count for key, count in declared.items()) or set(
            counts
        ) - set(declared):
        ...
        if num_samples is not None:
            if not 0 < num_samples <= len(filenames):
                raise ValueError(
                    f"Requested {num_samples} samples but {split} has {len(filenames)} eligible samples."
                )
        ...
        self.manifest = {
            ...
            "available_samples": eligible_counts[split],
            ...
            "oversampling_filter": {
                ...
                "eligible_split_counts": {
                    key: eligible_counts.get(key, 0) for key in declared
                },
                "excluded_split_counts": {
                    key: count - eligible_counts.get(key, 0)
                    for key, count in declared.items()
                },
            },
            "selection": (
                "manifest_order_prefix" if num_samples is not None else "all_eligible"
            ),
            ...
            "index_filter": {
                key: value
                for key, value in manifest.get("index_filter", {}).items()
            ...
            "refinement_targets": self.return_refinement_targets,
            "refinement_input": "sqrt(I) divided by its per-volume maximum",
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        with np.load(self.root / filename, allow_pickle=False) as stored:
            sample = prepare_author_training_sample(
                stored,
                input_log_data=self.input_log_data,
                shape=self.shape,
                return_refinement_targets=self.return_refinement_targets,
            )

# UMamba-Based AutoPhaseNN PyTorch Training Pipeline

This folder is a cleaned, standalone PyTorch training/evaluation flow for the
original UMamba model while keeping the AutoPhaseNN pipeline data, losses,
checkpointing, evaluation, and visualization logic.
...
& $PY "umamba_based_autophasenn\train.py" `
...
```bash
python umamba_based_autophasenn/train.py \
  --data-dir /data_ssd/oyys/autophasenn \
  --data-train-diff train_diff.npy \
  --data-train-real train_real.npy \
  --output-dir ./umamba_based_autophasenn/output/umamba_overfit100_l1 \
  --from-scratch \
  --loss-type l1 \
  --loss-scope diff \
  --overfit-samples 100 \
...
& $PY "umamba_based_autophasenn\train.py" `
  ...
  --pretrained "path\to\umamba_checkpoint.pt" `
  --device cuda `
  --batch-size 1 `
  --epochs 20 `
  --loss-type paper_mae `
...
```powershell
$PY = ".\.conda_autophase_tfpt\python.exe"
& $PY "umamba_based_autophasenn\evaluate.py" `
  --checkpoint "path\to\umamba_checkpoint.pt" `
  --data-dir "data\aicdi_sample\memmap" `
...
```powershell
$PY = ".\.conda_autophase_tfpt\python.exe"
& $PY "umamba_based_autophasenn\visualize_postprocessed.py" `
  --checkpoint "path\to\umamba_checkpoint.pt" `
  --data-dir "data\aicdi_sample\memmap" `
  --data-diff "val_diff.npy" `
  --data-real "val_real.npy" `

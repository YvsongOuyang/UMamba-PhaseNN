...
DEVICE="${DEVICE:-cuda}"
RUN_NAME="${RUN_NAME:-umamba_based_autophasenn_overfit${SAMPLES}_${LOSS_TYPE}_$(date +%Y%m%d_%H%M%S)}"
OUT_DIR="${OUT_DIR:-./umamba_based_autophasenn/output/${RUN_NAME}}"

echo "UMamba-based AutoPhaseNN-pipeline overfit test"
...
python umamba_based_autophasenn/evaluate.py \
...
python umamba_based_autophasenn/visualize_postprocessed.py \
  --checkpoint "${OUT_DIR}/checkpoint_best.pt" \
  --data-dir "${DATA_DIR}" \
  --data-diff train_diff.npy \
  --data-real train_real.npy \
...
print()
print("UMamba-based AutoPhaseNN-pipeline overfit summary")

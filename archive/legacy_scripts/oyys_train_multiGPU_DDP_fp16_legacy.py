...
def train(args, model, criterion, trainloader, optimizer, scheduler, epoch, scaler):
...
if __name__ == "__main__":
    ...
    kwargs = {'num_workers': args.num_workers, 'pin_memory': True, 'prefetch_factor': 2,'persistent_workers': True} 
    ...
    if args.lr_type == 'clr':
        iterations_per_epoch = np.floor((total_train_size*args.train_perc)/batch_size) + 1
    ...
    for epoch in range(epochs + 1, args.epoch+1):
        ...
        train_loss_details = train(args, model, criterion, train_loader, optimizer, scheduler, epoch, scaler)
        ...
        # 关键：用 global_step 替代 epoch，才能看到 CyclicLR 的变化

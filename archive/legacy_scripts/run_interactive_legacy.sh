...
# 记得把这里改成你新的 python 文件名
SCRIPT='oyys_lcrc_train_singleGPU.py'
...
save_model=10
n_workers=4  # <--- 修改了这里，单机推荐 8-32 之间
checkpoint='/data_ssd/oyys/autophasenn/Unsupfalse_D_U_T0.1_comb2_batch8_clr_Init1e-5_adam_scale1/best_model.pt'  # 断点续训模型路径，留空表示不使用断点续训
...
argument=''
argument_func $fp16 $unsupervise 
...
python $SCRIPT \
    ...
    --scale_I $scale_I 2>&1 | tee -a $result_path$output